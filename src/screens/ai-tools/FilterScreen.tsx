...
export default function FilterScreen() {
  ...
  const { selectedImage, isProcessing, resultUrl, error, taskError, pickImage, takePhoto, generate, clear } = useAiTool('filter');

  return (
    <SafeAreaView style={s.c}>
      <View style={s.h}><TouchableOpacity onPress={()=>n.goBack()}><Ionicons name="chevron-back" size={24} color={Colors.text} /></TouchableOpacity><View style={s.hc}><Text style={s.ht}>滤镜</Text><Text style={s.hs}>Filter</Text></View><View style={{width:24}} /></View>
      <ScrollView contentContainerStyle={s.ct}>
        ...
        {!resultUrl && <View style={s.dc}><Ionicons name="filter" size={24} color={Colors.primary} /><View style={{flex:1}}><Text style={s.dt}>滤镜</Text><Text style={s.de}>海量滤镜，一键套用</Text></View></View>}
