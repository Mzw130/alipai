...
export default function LipPlumpScreen() {
  const n = useNavigation();
  const { selectedImage, isProcessing, resultUrl, error, taskError, pickImage, takePhoto, generate, clear } = useAiTool('lip_plump');

  return (
    <SafeAreaView style={s.c}>
      <View style={s.h}><TouchableOpacity onPress={()=>n.goBack()}><Ionicons name="chevron-back" size={24} color={Colors.text} /></TouchableOpacity><View style={s.hc}><Text style={s.ht}>丰唇</Text><Text style={s.hs}>Lip Plump</Text></View><View style={{width:24}} /></View>
      <ScrollView contentContainerStyle={s.ct}>
        ...
        {!resultUrl && <View style={s.dc}><Ionicons name="rose" size={24} color={Colors.primary} /><View style={{flex:1}}><Text style={s.dt}>饱满嘴唇效果</Text><Text style={s.de}>AI 智能识别唇形，自然丰唇</Text></View></View>}
