...
export default function JawlineScreen() {
  const n = useNavigation();
  const { selectedImage, isProcessing, resultUrl, error, taskError, pickImage, takePhoto, generate, clear } = useAiTool('jawline');

  return (
    <SafeAreaView style={s.c}>
      <View style={s.h}><TouchableOpacity onPress={()=>n.goBack()}><Ionicons name="chevron-back" size={24} color={Colors.text} /></TouchableOpacity><View style={s.hc}><Text style={s.ht}>精致下颌线</Text><Text style={s.hs}>Jawline Contour</Text></View><View style={{width:24}} /></View>
      <ScrollView contentContainerStyle={s.ct}>
        ...
        {!resultUrl && <View style={s.dc}><Ionicons name="body" size={24} color={Colors.primary} /><View style={{flex:1}}><Text style={s.dt}>下巴轮廓优化</Text><Text style={s.de}>AI 智能塑形，精致下颌线条</Text></View></View>}
