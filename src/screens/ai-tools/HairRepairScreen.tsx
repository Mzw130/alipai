...
export default function HairRepairScreen() {
  ...
  const { selectedImage, isProcessing, resultUrl, error, taskError, pickImage, takePhoto, generate, clear } = useAiTool('hair_repair');

  return (
    <SafeAreaView style={s.c}>
      <View style={s.h}><TouchableOpacity onPress={()=>n.goBack()}><Ionicons name="chevron-back" size={24} color={Colors.text} /></TouchableOpacity><View style={s.hc}><Text style={s.ht}>发量修复</Text><Text style={s.hs}>Hair Repair</Text></View><View style={{width:24}} /></View>
      <ScrollView contentContainerStyle={s.ct}>
        ...
        {!resultUrl && <View style={s.dc}><Ionicons name="shield-checkmark" size={24} color={Colors.primary} /><View style={{flex:1}}><Text style={s.dt}>柔亮秀发效果</Text><Text style={s.de}>AI 修复发质，恢复柔亮光泽</Text></View></View>}
