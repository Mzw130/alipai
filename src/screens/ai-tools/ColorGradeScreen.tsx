...
export default function ColorGradeScreen() {
  const n = useNavigation();
  const { selectedImage, isProcessing, resultUrl, error, taskError, pickImage, takePhoto, generate, clear } = useAiTool('color_grade');

  return (
    <SafeAreaView style={s.c}>
      <View style={s.h}><TouchableOpacity onPress={()=>n.goBack()}><Ionicons name="chevron-back" size={24} color={Colors.text} /></TouchableOpacity><View style={s.hc}><Text style={s.ht}>调色</Text><Text style={s.hs}>Color Grade</Text></View><View style={{width:24}} /></View>
      <ScrollView contentContainerStyle={s.ct}>
        ...
        {!resultUrl && <View style={s.dc}><Ionicons name="contrast" size={24} color={Colors.primary} /><View style={{flex:1}}><Text style={s.dt}>调色</Text><Text style={s.de}>AI 智能调色，电影级色彩</Text></View></View>}
