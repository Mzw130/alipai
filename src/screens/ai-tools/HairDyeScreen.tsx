...
const HAIR_COLORS = [
  { name: 'Pink', color: '#FF69B4' }, { name: 'Red', color: '#FF4444' },
  { name: 'Gold', color: '#FFD700' }, { name: 'Brown', color: '#8B4513' },
];

export default function HairDyeScreen() {
  const n = useNavigation();
  const { selectedImage, isProcessing, resultUrl, error, taskError, pickImage, takePhoto, generate, clear } = useAiTool('hair_dye');
  const [selectedColor, setSelectedColor] = useState('Pink');
  return (
    <SafeAreaView style={s.c}>
      <View style={s.h}><TouchableOpacity onPress={()=>n.goBack()}><Ionicons name="chevron-back" size={24} color={Colors.text} /></TouchableOpacity><View style={s.hc}><Text style={s.ht}>染发</Text><Text style={s.hs}>Hair Dye</Text></View><View style={{width:24}} /></View>
      <ScrollView contentContainerStyle={s.ct}>
        ...
        <Text style={s.st}>选择发色</Text>
        <View style={s.cr}>{HAIR_COLORS.map((c)=>(<TouchableOpacity key={c.name} style={[s.ci,selectedColor===c.name&&{borderColor:Colors.primary}]} onPress={()=>setSelectedColor(c.name)}><View style={[s.cd,{backgroundColor:c.color}]} /><Text style={[s.cn,selectedColor===c.name&&{color:Colors.primary,fontWeight:FontWeight.semibold}]}>{c.name}</Text></TouchableOpacity>))}</View>
        <TouchableOpacity style={[s.gb,(!selectedImage||isProcessing)&&s.gbd]} onPress={()=>generate({hair_color:selectedColor})} disabled={!selectedImage||isProcessing}><Ionicons name="color-wand" size={20} color={Colors.text} /><Text style={s.gt}>{isProcessing?'处理中...':'生成'}</Text></TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
}
const s = StyleSheet.create({
  ...
  st:{fontSize:FontSize.md,fontWeight:FontWeight.semibold,color:Colors.text,marginBottom:Spacing.md},cr:{flexDirection:'row',gap:Spacing.md,marginBottom:Spacing['2xl']},ci:{flex:1,backgroundColor:Colors.card,borderRadius:BorderRadius.md,padding:Spacing.md,alignItems:'center',gap:Spacing.sm,borderWidth:1,borderColor:Colors.border},cd:{width:32,height:32,borderRadius:16},cn:{fontSize:FontSize.xs,color:Colors.textMuted},
