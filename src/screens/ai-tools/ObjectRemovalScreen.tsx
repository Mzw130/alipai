    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>物体消除</Text>
          <Text style={styles.headerSub}>Object Removal</Text>
        </View>
        <View style={{ width: 24 }} />
      </View>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.descCard}>
          ...
          <Text style={styles.descText}>手动涂抹移除区域，AI 智能消除图片内指定物体</Text>
        </View>
        <View style={styles.toolRow}>
          {['涂抹', '绘图', '擦除', '重置'].map((name, i) => (
            <TouchableOpacity key={i} style={styles.toolBtn} activeOpacity={0.7}>
              <Ionicons name={['brush','pencil','eraser','refresh'][i] as any} size={20} color={Colors.textSecondary} />
        ...
        <TouchableOpacity style={styles.generateBtn} onPress={() => setShowAuth(true)} activeOpacity={0.8}>
          ...
          <Text style={styles.generateText}>移除</Text>
        </TouchableOpacity>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  descCard: { backgroundColor: Colors.card, borderRadius: BorderRadius.md, padding: Spacing.lg, flexDirection: 'row', alignItems: 'center', gap: Spacing.md, marginBottom: Spacing.lg, borderWidth: 1, borderColor: Colors.border },
  descText: { fontSize: FontSize.base, color: Colors.textSecondary, flex: 1, lineHeight: 22 },
  toolRow: { flexDirection: 'row', gap: Spacing.sm, marginBottom: Spacing['2xl'] },
  toolBtn: { flex: 1, backgroundColor: Colors.card, borderRadius: BorderRadius.md, padding: Spacing.md, alignItems: 'center', gap: Spacing.xs, borderWidth: 1, borderColor: Colors.border },
