...
export default function ObjectRemovalScreen() {
  ...
  const { selectedImage, isProcessing, resultUrl, error, taskError, pickImage, takePhoto, generate, clear } = useAiTool('obj_remove');

  return (
    <SafeAreaView style={s.c}>
      <View style={s.h}><TouchableOpacity onPress={()=>n.goBack()}><Ionicons name="chevron-back" size={24} color={Colors.text} /></TouchableOpacity><View style={s.hc}><Text style={s.ht}>物体消除</Text><Text style={s.hs}>Object Removal</Text></View><View style={{width:24}} /></View>
      <ScrollView contentContainerStyle={s.ct}>
        ...
        {!resultUrl && <View style={s.dc}><Ionicons name="hand-left" size={24} color={Colors.primary} /><View style={{flex:1}}><Text style={s.dt}>物体消除</Text><Text style={s.de}>手动涂抹移除区域，AI 智能消除图片内指定物体</Text></View></View>}
